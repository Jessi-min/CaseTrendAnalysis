...
DEFAULT_METRIC = "Adjusted" # Retain as Adjusted as per original intent
...
def _generate_fluctuation_report_html(combined_df_long: pd.DataFrame, metrics_to_analyze: List[str]) -> str:
    ...
    Fluctuation is now measured by Variance.
    """
    report_parts = []
...
def analyze_fluctuations(df: pd.DataFrame, out_path: Path, metric_name: str):
    """
    Analyzes fluctuation (mean, standard deviation, variance) for a specific metric across use cases.
    Saves the results to CSV and prints top 10. This is the version called for external CSV.
    """
    logger.info(f"Analyzing fluctuation for metric '{metric_name}' (using Variance)...") # MODIFICATION: Updated log message
    ...
    try:
        ...
        # MODIFICATION: Updated log messages for variance
        ...
        logger.info(fluctuation_data.head(10).round(4).to_string(index=False)) # Display top 10 by Variance
        logger.info("-----------------------------------------------------")
...
def main():
    ...
    ap.add_argument(
        ...
        # MODIFICATION: Updated help text for variance
    ...
    if is_china_special_case and all_processed_dfs:
        ...
        if args.analyze_fluctuation:
            if DEFAULT_METRIC in combined_df_long["Metric"].unique(): # Check if 'Adjusted' is among the collected metrics
                # MODIFICATION: Changed filename to reflect Variance
                fluctuation_output_path = output_base_dir / f"aggregate_China_{DEFAULT_METRIC}_variance_fluctuation.csv"
